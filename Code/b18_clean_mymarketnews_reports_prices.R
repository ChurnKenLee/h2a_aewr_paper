...
for (year in 2004:2012) {
  ...
  # Define price as midpoint of high and low price when available
  terminal_market <- terminal_market %>%
    mutate(price = if_else(
      (!is.na(mostly_high_price) & !is.na(mostly_low_price)), (mostly_high_price + mostly_low_price)/2, NA
    )) %>%
    mutate(price = if_else(
      (is.na(price) & (!is.na(high_price) & !is.na(low_price))), (high_price + low_price)/2, price
    ))

  # Some observations only has low price or high price; use these as price
  terminal_market <- terminal_market %>%

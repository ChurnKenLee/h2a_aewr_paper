...
# Remove addendum B cases with not corresponding master entry, as we don't have the certified worker count
...
master_entries <- h2a_duped_pre2020 %>%
  filter(entry_type == "master") %>%
  mutate(master_n_workers = nbr_workers_certified) %>% 
  mutate(master_n_workers_requested = nbr_workers_requested) %>% 
  select(case_number, fiscal_year, master_n_workers, master_n_workers_requested, organization_flag) %>% 
...
sub_entries <- h2a_duped_pre2020 %>%
  ...
  group_by(case_number, fiscal_year) %>%
  mutate(sub_n_workers = sum(nbr_workers_certified, na.rm = TRUE)) %>% 
  mutate(sub_n_workers_requested = sum(nbr_workers_requested, na.rm = TRUE)) %>% 
  ungroup() %>%
  select(case_number, fiscal_year, sub_n_workers, sub_n_workers_requested) %>%
...
# Master entry has the true count of the number of workers
# We have to use application type to correctly apportion workers across entries
...
# Likewise for applications tagged as "Master", M
...
h2a_duped_pre2020 <- h2a_duped_pre2020 %>% 
  mutate(master_sub_ratio = master_n_workers/sub_n_workers) %>% 
  mutate(worker_count_diff = master_n_workers - sub_n_workers) %>% 
  mutate(adjusted_nbr_workers_certified = case_when(
    application_type %in% drop_master_list & sub_n_workers > 0 ~ nbr_workers_certified*master_sub_ratio,
    application_type %in% drop_master_list & sub_n_workers == 0 ~ master_n_workers/(n_entries - 1),
    !(application_type %in% drop_master_list) & sub_n_workers == 0 ~ master_n_workers/n_entries, # Evenly split worker counts across all entries including master entry, since all sub entries are NA
    !(application_type %in% drop_master_list) & sub_n_workers > 0 & master_n_workers >= sub_n_workers & entry_type == "master" ~ nbr_workers_certified - sub_n_workers, # master >= sub total, remove sub worker count from master entry since we want to keep master entry
    !(application_type %in% drop_master_list) & sub_n_workers > 0 & master_n_workers >= sub_n_workers & entry_type == "sub" ~ nbr_workers_certified,  # master >= sub total, keep sub entries the same
    !(application_type %in% drop_master_list) & sub_n_workers > 0 & master_n_workers < sub_n_workers ~ master_n_workers/n_entries # master < sub total, just use (# workers)/(# of entries)
    ))

h2a_duped_pre2020 <- h2a_duped_pre2020 %>% 
  mutate(master_sub_requested_ratio = master_n_workers_requested/sub_n_workers_requested) %>% 
  mutate(worker_count_requested_diff = master_n_workers_requested - sub_n_workers_requested) %>% 
  mutate(adjusted_nbr_workers_requested = case_when(
    application_type %in% drop_master_list & sub_n_workers_requested > 0 ~ nbr_workers_requested*master_sub_requested_ratio,
    application_type %in% drop_master_list & sub_n_workers_requested == 0 ~ master_n_workers_requested/(n_entries - 1),
    !(application_type %in% drop_master_list) & sub_n_workers_requested == 0 ~ master_n_workers_requested/n_entries, # Evenly split worker counts across all entries including master entry, since all sub entries are NA
    !(application_type %in% drop_master_list) & sub_n_workers_requested > 0 & master_n_workers_requested >= sub_n_workers_requested & entry_type == "master" ~ nbr_workers_requested - sub_n_workers_requested, # master >= sub total, remove sub worker count from master entry since we want to keep master entry
    !(application_type %in% drop_master_list) & sub_n_workers_requested > 0 & master_n_workers_requested >= sub_n_workers_requested & entry_type == "sub" ~ nbr_workers_requested,  # master >= sub total, keep sub entries the same
    !(application_type %in% drop_master_list) & sub_n_workers_requested > 0 & master_n_workers_requested < sub_n_workers_requested ~ master_n_workers_requested/n_entries # master < sub total, just use (# workers)/(# of entries)
  ))
...
h2a_duped_post2020 <- h2a_duped_post2020 %>%
  group_by(case_number, fiscal_year) %>% 
  mutate(mean_nbr_workers_requested = mean(nbr_workers_requested, na.rm = TRUE)) %>% 
  ungroup() %>% 
  mutate(missing_nbr_workers_requested = is.na(nbr_workers_requested)) %>% 
  mutate(nbr_workers_requested = case_when(
    missing_nbr_workers_requested ~ mean_nbr_workers_requested,
    .default = nbr_workers_requested
  ))
# Note that this produces equal weights for cases with all addendum B worker counts missing

# Start by dropping HQ location of association applications
application_types <- h2a_duped_post2020 %>% 
  distinct(type_of_employer_application)
...
test <- h2a_duped_post2020 %>% 
  filter(is.na(case_nbr_workers_certified))
  
h2a_duped_post2020 <- h2a_duped_post2020 %>% 
  filter((type_of_employer_application %in% keep_master_list) | 
           (type_of_employer_application %in% drop_master_list & entry_type == "sub"))

# Calculate weights and produce worksite worker counts
h2a_duped_post2020 <- h2a_duped_post2020 %>%
  group_by(case_number, fiscal_year) %>%
  mutate(case_total_nbr_workers_requested = sum(nbr_workers_requested, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(share_of_case_total_workers_requested = nbr_workers_requested/case_total_nbr_workers_requested) %>%
  mutate(adjusted_nbr_workers_certified = case_nbr_workers_certified*share_of_case_total_workers_requested) %>%
  mutate(adjusted_nbr_workers_requested = case_nbr_workers_requested*share_of_case_total_workers_requested)

# Now combine all back together
...
h2a_combined <- h2a_combined %>%
  mutate(begin_date = case_when(
    case_number == "H-300-13135-069256" ~ "2013-07-01",
    case_number == "H-300-16082-567927" ~ "2016-03-25",
    case_number == "H-300-18141-758942" ~ "2018-05-22",
    case_number == "H-300-18325-715722" ~ "2018-11-21",
    case_number == "H-300-19017-003849" ~ "2019-01-22",
    case_number == "H-300-19044-363875" ~ "2019-03-08",
    case_number == "H-300-19090-126428" ~ "2019-03-31",
    case_number == "H-300-19207-740828" ~ "2019-08-23",
    case_number == "H-300-18198-221259" ~ "2018-07-17",
    case_number == "H-300-18229-549625" ~ "2018-09-13",
    .default = begin_date
  ))
...
h2a_combined <- h2a_combined %>%
  mutate(end_date = case_when(
    case_number == "H-300-16011-375074" ~ "2016-07-21",
    case_number == "H-300-19034-745829" ~ "2019-08-17",
    .default = end_date
  ))
...
errors <- h2a_combined %>%
  ...
  mutate(begin_date_error = begin_year < as.numeric(fiscal_year) - 1) %>%
  mutate(end_year = year(posix_end_date)) %>%
  mutate(end_date_error = end_year < as.numeric(fiscal_year) - 1) %>%
  group_by(case_number, fiscal_year) %>%
  mutate(case_has_date_error = any(begin_date_error) | any(end_date_error)) %>%
  ungroup() %>%
  filter(case_has_date_error) %>%
  arrange(fiscal_year, case_number)

# These 226 cases have a start date after the end date; drop these
h2a_combined <- h2a_combined %>%
...
# Sanity check numbers mostly match (dropped entries with no dates), we are good
...
h2a_df <- read_parquet(here("binaries", "h2a_cleaned.parquet"))
...
test <- h2a_df %>% filter(wage_rate == 0)

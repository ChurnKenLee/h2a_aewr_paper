...
| \(b_m(a)\): binding exposure | `cert_hours_at_aewr_start_year` divided by `cert_hours_with_hourly_wage_and_aewr_start_year`; an offer is treated as AEWR-exposed when its explicit hourly rate is within $0.05 of the contract start-year or prior-year state AEWR | Constructed in `data/intermediate/h2a_aggregated.parquet`; explicit-hourly and AEWR-match coverage are retained separately | Required for the mechanical wage exposure. This is an exposure proxy, not proof that the AEWR was the legally binding floor: the disclosure data do not identify every prevailing, collective-bargaining, federal, or state wage floor. |
...
| Baseline offered wage bill | Explicit hourly offer × certified contract hours, summed as `nominal_offered_wage_bill_certified_start_year` | Constructed in `data/intermediate/h2a_aggregated.parquet`; `cert_hours_with_hourly_wage_start_year` records its coverage | Certification-space wage obligation for certified H-2A positions, not observed payroll. Multiply by the realized-to-certified-hours scenario \(\rho_{mt}\) for realized earnings incidence. |

# Purpose: Inspect RMA Actuarial Data Master price records and layouts.
# Inputs: RMA ADM archives under data/raw/risk_management_agency.
# Outputs: none; this exploratory source-preparation app currently writes no artifact.
...
def _():
    ...
    return RAW, ZipFile, io, mo, pio, pl, re


@app.cell
def _(RAW):
    adm_path = RAW / "risk_management_agency" / "actuarial_data_master"
    return (adm_path,)
...
def _(ZipFile, io, re):
    ...
    def extract_00_10_adm_col_spec(
        meta_path,
        toc_file_name,
        col_spec_file_name,
        toc_encoding,
        col_spec_encoding,
        target_record_purpose,
    ):
        ...
        for line in toc_content.splitlines():
            ...
            remainder_str = line[
                17:
            ]  # There are >=5 blank spaces between record purpose and page number
            ...
            if len(split) > 1:
                ...
                if record_purpose_str == target_record_purpose:
                    print("Found TOC row")
                    print(
                        f"Prefix = {prefix_str}, record purpose = {record_purpose_str}, page = {page_str}"
                    )
                    break

        col_spec_file = meta_zip.open(col_spec_file_name)
        col_spec_content = io.TextIOWrapper(
            col_spec_file, encoding=col_spec_encoding
        ).read()
...
def _(ZipFile, adm_path, extract_00_10_adm_col_spec, pio):
    ...
    for y in range(0, 11):
        ...
        col_specs, prefix_str = extract_00_10_adm_col_spec(
            meta_path,
            toc_file_name,
            col_spec_file_name,
            "cp1252",
        ...
        for col_spec in col_specs:
            fwf_dict[col_spec["field_name"]] = (
                col_spec["start"] - 1,
                col_spec["start"] - 1 + col_spec["length"],
            )
        ...
        prices_00_10_dfs[year] = pio.read_fwf(
            data_file, cols=fwf_dict, infer_schema=False, encoding="cp1252"
        )
        ...
        data_zip = ZipFile(data_path, "r")
        data_file = data_zip.open(f"R{prefix}{year}YTD").read()
        cross_reference_00_10_dfs[year] = pio.read_fwf(
            data_file, cols=fwf_dict, infer_schema=False, encoding="cp1252"
        )
    return
...
def _(arc_zip, layout_file_name, pl):
    ...
    layout_df = pl.read_csv(
        layout_file.read(), separator="|", infer_schema=False, encoding="1252"
    )
    return (layout_df,)
...
@app.cell
def _(layout_df, pl):
    price_layout = layout_df.filter(pl.col("ADM Record Type Code") == "A00810")
    return
...
def _(arc_zip, pl, price_file_name):
    ...
    price_df = pl.read_csv(
        price_file.read(), separator="|", infer_schema=False, encoding="1252"
    )
    return

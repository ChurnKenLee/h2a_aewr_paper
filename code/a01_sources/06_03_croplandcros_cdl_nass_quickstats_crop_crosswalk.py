# Purpose: Build candidate mappings between NASS commodities and CDL crop classes.
# Inputs: NASS selection metadata, CDL/FSA crosswalks, and GOOGLE_GEMINI_API_KEY.
# Outputs: data/intermediate/cache/nass_to_cdl_mappings.jsonl
...
def _(Path, cdl_fsa_code_dict, cdl_path, pl):
    ...
    cdl_fsa_xwalk = cdl_fsa_xwalk.group_by("cdl_code").agg(
        [
            pl.col("fsa_crop_name")
            .str.split(",")
            .list.explode()
            .str.strip_chars()
            .unique()
            ...
            .implode()
            .alias("fsa_types"),
            pl.col("fsa_intended_use")
            .str.split(",")
            .list.explode()
            .str.strip_chars()
            .unique()
            .implode()
            .alias("fsa_intended_use_code"),
        ]
    )
    ...
    cdl_fsa_xwalk = cdl_fsa_xwalk.with_columns(
        ...
        .list.eval(
            pl.element().replace_strict(cdl_fsa_code_dict, default=None).drop_nulls()
        )
        .alias("fsa_intended_use")
    ).select(["cdl_code", "fsa_crops", "fsa_types", "fsa_intended_use"])
...
def _(cdl_path, pdfplumber, pl):
    ...
    cdl_fsa_code_dict = dict(
        cdl_fsa_mapping.select("fsa_use_code", "fsa_use_name").iter_rows()
    )
...
def _(BaseModel, Field, List):
    ...
    class CDLMatchOutput(BaseModel):
        ...
        codes: List[int] = Field(
            description="List of integer CDL codes (e.g., [1, 26, 225]). Include all relevant primary and double-crop codes."
        )
...
def _(CDLMatchOutput, dspy):
    class NASSCDLMatcherSignature(dspy.Signature):
        ...
        nass_canonical_definition = dspy.InputField(
            desc="The hierarchical NASS crop definition. From most general to most specific categorization: GROUP, then COMMODITY, then CLASS. PRODUCTION PRACTICE and UTILIZATION PRACTICE provide additional detail. Semantic hint is a guess at the appropriate common crop name."
        )

        cdl_table = dspy.InputField(
            desc="The full list of CDL codes and names, with added FSA metadata"
        )

        answer: CDLMatchOutput = dspy.OutputField(
            desc="The final structured mapping result."
        )
...
def _(NASSCDLMatcherSignature, dspy, query_fsa_metadata):
    ...
    matcher_agent = dspy.ReAct(
        NASSCDLMatcherSignature, tools=[query_fsa_metadata], max_iters=20
    )
...
def _(
    ...
    print(
        f"Resuming: {len(processed_ids)} already processed. Total to process: {len(nass_obs_selection)}"
    )
    ...
    for nass_id, nass_entry in tqdm.tqdm(islice(nass_obs_selection.items(), 300)):
        ...
        try:
            ...
            prediction = matcher_agent(
                nass_canonical_definition=nass_yaml, cdl_table=cdl_fsa_markdown
            )

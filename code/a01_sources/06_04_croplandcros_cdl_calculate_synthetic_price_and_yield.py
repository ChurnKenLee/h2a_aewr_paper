# Purpose: Construct synthetic CDL price and yield series from matched NASS crops.
# Inputs: NASS survey selections, candidate mappings, unit mappings, and Gemini.
# Outputs: state and national cdl_price_yield_synthetic_*.parquet artifacts.
...
@app.cell
def _(CACHE, INTERMEDIATE, RAW, dspy, os):
    ...
    return binary_path, cdl_path, json_path
...
def _(Path, cdl_path, json_path, pl):
    ...
    cdl_names = pl.read_excel(
        Path(cdl_path / "CDL_codes_names_colors.xlsx"), read_options={"header_row": 3}
    )
    cdl_names = (
        cdl_names.rename(str.lower)
        .select([pl.col("codes"), pl.col("class_names")])
        .rename(mapping={"codes": "cdl_code", "class_names": "cdl_name"})
    )
    ...
    df_results = df_results.with_columns(cdl_code=pl.col("mapping").struct.field("codes"))
    df_results = df_results.with_columns(
        cdl_name=pl.col("cdl_code").list.eval(pl.element().replace_strict(cdl_code_lookup))
    ).explode(["cdl_code", "cdl_name"])
    return (df_results,)
...
def _(binary_path, df_results, nass_obs_selection):
    ...
    nass_cdl_xwalk = nass_obs_selection.join(nass_cdl_xwalk, how="left", on=["nass_id"])
...
def _(binary_path, pl):
    ...
    qs_survey_crops = qs_survey_crops.filter(
        pl.col("observation_type") != "", ~pl.col("value").is_null()
    )
...
def _(mo):
    mo.md(r"""
    ```python
    # This generates the specification of what type of data is contained within each unique observation_type X unit_desc
    unique_unit_desc = qs_survey_crops.select("observation_type", "unit_desc").unique()
    obs_type_list = unique_unit_desc.get_column("observation_type").unique().to_list()

    mapping = {}

    # Handle observation_type strings
    for obs_type in obs_type_list:
        mapping[obs_type] = {"string_type": "observation_type", "dimension": obs_type}

    # Handle unit_desc strings
    for row in unique_unit_desc.iter_rows():
        obs_type = row[0]
        unit_desc = row[1]
    ...
    with open(CACHE / "crop_unit_mapping.json", "w", encoding="utf-8") as _f:
        json.dump(mapping, _f, indent=2)
    ```
    """)
    return
...
@app.cell
def _(CACHE, json, pl, qs_survey_crops):
    # 1. Load the mapping specification generated earlier
    with open(CACHE / "crop_unit_mapping.json", "r") as f:
        mapping = json.load(f)
    ...
    money_unit_map = {
        k: v["money_unit"] for k, v in mapping.items() if v["string_type"] == "unit_desc"
    }
    weight_unit_map = {
        k: v["weight_unit"] for k, v in mapping.items() if v["string_type"] == "unit_desc"
    }
    area_unit_map = {
        k: v["area_unit"] for k, v in mapping.items() if v["string_type"] == "unit_desc"
    }
    basis_map = {
        k: v["basis"] for k, v in mapping.items() if v["string_type"] == "unit_desc"
    }
...
def _():
    ...
    money_unit_common_name = {"$": "Dollar", "CENTS": "Cent"}
    return (
        area_unit_common_name,
...
def _(pl, qs_harmonized_unit_name):
    ...
    unique_pairs = (
        qs_harmonized_unit_name.select(["commodity_desc", "weight_unit"])
        .unique()
...
def _(BaseModel, Field):
    ...
    class WeightConversionOutput(BaseModel):
        ...
        conversion_factor: float = Field(
            description="Multiplier to convert given unit into pounds."
        )

        # This captures the 'Why' for auditing
        reasoning: str = Field(
            description="Brief justification for the conversion multiplier chosen."
        )
...
def _(WeightConversionOutput, dspy):
    ...
    class AgUnitConversion(dspy.Signature):
        ...
        crop_name = dspy.InputField(
            desc="The NASS commodity name (e.g., CORN, APPLES, PEACHES)"
        )
        unit_name = dspy.InputField(
            desc="The harmonized unit name (e.g., Bushel, Basket, Barrel, Box)"
        )
...
def _(conversion_results, pl, qs_harmonized_unit_name):
    # Join final list back to NASS
    unit_conversion_lookup = pl.DataFrame(conversion_results)
    qs_final_standardized = qs_harmonized_unit_name.join(
        unit_conversion_lookup, on=["commodity_desc", "weight_unit"], how="left"
    ).with_columns(
        ...
        .when(  # Area in sq ft, mostly horticulture, mushrooms
            (pl.col("observation_type") == "area") & (pl.col("area_unit") == "Square Foot")
        )
        .then(pl.col("value") / 43560)
        .when(  # Acreage in acres already
            (pl.col("observation_type") == "area") & (pl.col("area_unit") == "Acre")
        )
        .then(pl.col("value") * 1)
...
def _(nass_cdl_xwalk, pl, qs_final_standardized):
    ...
    qs_final_standardized.drop(
        "source_desc", "domain_desc", "domaincat_desc", "cv_%"
    ).filter(pl.col("standardized_value") == 999999)

    # Update unit labels
    qs_cleaned = (
        qs_final_standardized.filter(pl.col("standardized_value") != 999999)
        .drop_nulls(subset=["standardized_value"])
        # Standardize the unit labels for the final time
        .with_columns(
            pl.when(pl.col("weight_unit").is_not_null())
            .then(pl.lit("Pound"))
            .when(pl.col("area_unit").is_not_null())
            .then(pl.lit("Acre"))
            .when(pl.col("money_unit") == "Dollar")
            .then(pl.lit("USD"))
...
def _(pivot_index, pl, qs_cleaned):
    ...
    qs_state_wide = (
        qs_state_only.with_columns(
            obs_key=pl.when(
                (pl.col("observation_type") == "production")
                & (pl.col("physical_dimension") == "USD")
            )
...
def _(copy, nass_identity, pivot_index, pl, qs_cleaned):
    ...
    qs_national_wide = (
        ...
        .select(
            ["year", "cdl_code"]
            + nass_identity
            + ["y_national", "p_national", "v_national", "q_national"]
        )
    )
...
def _(nass_identity, pl, qs_national_wide, qs_state_wide):
    ...
    qs_state_revenue = (
        qs_state_wide.join(
            qs_national_wide.drop("cdl_code"), on=nass_identity + ["year"], how="left"
        )
        ...
        .with_columns(
            revenue=pl.coalesce([pl.col("eff_v"), pl.col("eff_p") * pl.col("eff_q")])
        )
    )
...
def _(binary_path, pl, qs_national_wide):
    ...
    national_synthetic_cdl = (
        qs_national_wide.with_columns(
            revenue_nat=pl.coalesce(
                [pl.col("v_national"), pl.col("p_national") * pl.col("q_national")]
            )
        )
        .drop_nulls(subset=["revenue_nat", "p_national", "y_national"])
        .with_columns(
            w_nat=pl.col("revenue_nat")
            / pl.col("revenue_nat").sum().over(["year", "cdl_code"])
        )
        .with_columns(
            [
        ...
        .agg([pl.sum("p_w_nat").alias("p_syn_nat"), pl.sum("y_w_nat").alias("y_syn_nat")])
    )
    national_synthetic_cdl.write_parquet(

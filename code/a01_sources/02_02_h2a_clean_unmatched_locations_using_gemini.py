# Purpose: Suggest cleaned place names for H-2A locations not matched deterministically.
# Inputs: unmatched H-2A location CSVs and GOOGLE_GEMINI_API_KEY.
# Outputs: unmatched location suggestion CSVs and cached batch responses.
...
@app.cell
def _(CACHE, dotenv, os):
    json_path = CACHE
    dotenv.load_dotenv()
    gemini_api_key = os.getenv("GOOGLE_GEMINI_API_KEY")
    return gemini_api_key, json_path
...
def _(BaseModel, Field, Optional, get_args, get_origin):
    ...
    class CleanedLocation(BaseModel):
        location_hash: str = Field(
            ..., description="The unique hash ID of the row from the input data."
        )
        city: Optional[str] = Field(None, description="Corrected city name.")
        county: Optional[str] = Field(
            None, description="County name if present or easily inferable."
        )
        state: Optional[str] = Field(
            None, description="Two-letter state abbreviation (e.g., NY, CA)."
        )
    ...
    def to_gemini_schema(model_class):
        ...
        for name, field in model_class.model_fields.items():
            ...
            if (
                ...
                or str(field_type).startswith("typing.Optional")
            ):
            ...
            properties[name] = {"type": gemini_type, "description": field.description}
    ...
    def write_batch_request_jsonl(
        ...
        for i in range(0, len(df), chunk_size):
            ...
            request_entry = {
                ...
                "request": {
                    "contents": [{"parts": [{"text": prompt_string}], "role": "user"}],
                    "generation_config": {
    ...
    def upload_jsonl_using_files_api(jsonl_path, upload_display_name):
        ...
        uploaded_file = client.files.upload(
            ...
            config=types.UploadFileConfig(
                display_name=upload_display_name, mime_type="jsonl"
            ),
        )
...
def _(client, time):
    ...
    def poll_genai_job(job_name, sleep_duration, output_file_name):
        ...
        while batch_job.state.name not in completed_states:
            print(
                f"Job current state: {batch_job.state.name}, waiting {sleep_duration} seconds"
            )
        ...
        with open(output_file_name, "w", encoding="utf-8") as _f:
...
def _(
    ...
    def process_gemini_batch_with_cache(
        ...
        if len(new_df) == 0:
            print(
                f"All locations already cached for {job_display_name}! Skipping API calls."
            )
            return existing_results_df

        print(
            f"Found {len(new_df)} new locations to process. Initiating Gemini Batch Job..."
        )

        # 3. Create request file for NEW rows
        write_batch_request_jsonl(
            new_df, hash_col, genai_schema, request_jsonl_path, batch_rows
        )
        ...
        temp_output_file = request_jsonl_path.with_name(
            f"temp_output_{job_display_name}.txt"
        )
...
@app.cell
def _(CleanedLocation, Path, ValidationError, json, pl):
    ...
    def parse_results(response_string_file):
        ...
        with open(response_string_file, "r", encoding="utf-8") as f:
            lines = f.readlines()
        ...
        for l in lines:
            ...
            if "response" not in batch_item:
                print(
                    f"Skipping {chunk_key}: No response found (Status: {batch_item.get('status')})"
                )
                continue
...
def _(Path):
    ...
    return clear_job_id, load_job_id, save_job_id
...
def _(
    ...
    def submit_gemini_batch_async(
        ...
        if existing_job_id:
            print(
                f"A job is already pending: {existing_job_id}. Please check its status first."
            )
...
@app.cell
def _(Path, clear_job_id, client, load_job_id, parse_results, pl):
    def check_and_retrieve_gemini_batch(
        cache_file: Path, tracking_file: Path
    ) -> pl.DataFrame:
    ...
    return (check_and_retrieve_gemini_batch,)


@app.cell
def _(Path, get_location_hash, json_path, pl):
    h2a_df = pl.read_csv(json_path / "unmatched_h2a_locations.csv").fill_null("")
    h2a_df_copy = h2a_df.with_columns(
        pl.struct(["city", "county", "state", "zip"])
        .map_elements(
    ...
    return h2a_cache_file, h2a_df_copy, h2a_jsonl_request, h2a_tracking_file
...
def _(
    Path,
    check_and_retrieve_gemini_batch,
    h2a_cache_file,
    h2a_df_copy,
    h2a_tracking_file,
    json_path,
    ...
    h2a_results_df = check_and_retrieve_gemini_batch(
        cache_file=h2a_cache_file, tracking_file=h2a_tracking_file
    )

    # Merge back with the unmatched locations
    # (If the job is still running, this just merges whatever we already had)
    h2a_with_suggestions = (
        h2a_df_copy.join(
            h2a_results_df, on="location_hash", how="left", suffix="_suggested"
        )
        .rename({"zipcode": "zip_suggested", "confidence": "suggestion_confidence"})
        .drop("location_hash")
    )
...
def _(
    ...
    add_b_results_df = check_and_retrieve_gemini_batch(
        cache_file=add_b_cache_file, tracking_file=add_b_tracking_file
    )

    # Merge back with the unmatched locations
    # (If the job is still running, this just merges whatever we already had)
    add_b_with_suggestions = (
        add_b_df_copy.join(
            add_b_results_df, on="location_hash", how="left", suffix="_suggested"
        )
        .rename({"zipcode": "zip_suggested", "confidence": "suggestion_confidence"})
        .drop("location_hash")
    )

    add_b_with_suggestions.write_csv(
        Path(json_path / "unmatched_add_b_with_suggestions.csv")
    )
    return

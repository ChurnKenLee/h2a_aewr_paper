...
# Purpose: Convert raw NASS QuickStats extracts into typed source partitions.
# Inputs: raw NASS QuickStats files under data/raw/quickstats.
# Outputs: qs_census_*.parquet and qs_survey_*.parquet in data/intermediate.
...
def _():
    ...
    return CACHE, INTERMEDIATE, RAW, mo, pl
...
def _(CACHE, INTERMEDIATE, RAW):
    ...
    json_path = CACHE
    quickstats_path = RAW / "quickstats"
    ...
    for qs_type in nass_qs_list:
        for file_path in quickstats_path.iterdir():
            ...
            if qs_type in file_name:
                ...
                survey_df = df.filter(pl.col("source_desc") == "SURVEY")
                survey_df.write_parquet(binary_path / f"qs_survey_{qs_type}.parquet")
    return

# Purpose: Assemble historical adverse effect wage rates from published tables.
# Inputs: AEWR source files in data/raw/aewr and Federal Register metadata.
# Outputs: data/intermediate/aewr.parquet
...
def _(aewr_path):
    crs_report = (
        aewr_path / "20080326_RL32861_9a486634f79a5f9c680cc5ba021e579cc67210a5.pdf"
    )
    return (crs_report,)
...
def _(aewr_path, pl):
    ...
    cfr_csv = pl.read_csv(
        aewr_path
        / "documents_matching_labor_certification_process_adverse_effect_wage_rates_from_labor_department_and_of_type_notice.csv"
    )
    ...
    aewr_2009_csv = pl.read_csv(aewr_path / "documents_matching_2010_2731.csv")
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                policy_year = (
                    year_match.group(0)
                    if year_match
                    else data.get("publication_date")[:4]
                )
...
def _(BeautifulSoup):
    ...
    def parse_aewr_table(xml_path, policy_year, doc_num):
        ...
        for table in soup.find_all("GPOTABLE"):
            ...
            if "STATE" not in header_text or (
                "AEWR" not in header_text and "WAGE" not in header_text
            ):
                continue
...
def _(BeautifulSoup, pl, us, xml_path):
    ...
    for _table in soup.find_all("GPOTABLE"):
        ...
        if "STATE" not in _header_text or (
            "AEWR" not in _header_text and "WAGE" not in _header_text
        ):
            continue
...
def _(aewr_2009, all_data, pl, us):
    ...
    aewr_df = aewr_df.with_columns(
        ...
        pl.when((pl.col("state") == "Hawaii") & (pl.col("policy_year") == 2003))
        .then(pl.lit(9.42))
        .otherwise(pl.col("aewr"))
        .alias("aewr"),
    )

    # Doc 2024-29549 is actually for 2025 but does not have year in the table name for some reason
    aewr_df = aewr_df.with_columns(
        pl.when(pl.col("document_number") == "2024-29549")
        .then(pl.lit(2025))
        .otherwise(pl.col("policy_year"))
        .alias("policy_year")
    )
...
def _(aewr_df, crs_aewr_long, name_fips_dict, pl):
    # Use mutually exclusive source ranges: CRS through 2008 and Federal
    # Register tables from 2009 onward. The Federal Register search results
    ...
    aewr_1990_2025 = pl.concat([crs_aewr, federal_register_aewr]).sort(
        ["state_name", "year"]
    )
...
def _(aewr_1990_2025, aewr_path, pl):
    ...
    usda_aewr = (
        usda_aewr.unpivot(
            index=["State", "Region"], variable_name="year", value_name="usda_aewr"
        )
        .rename(
            {
                "State": "state_name",
...
@app.cell
def _(INTERMEDIATE, check_df, pl):
...
def _(aewr_1990_2025, alt, pl):
    ...
    agg_exprs = [
        pl.col("aewr").quantile(q).alias(f"q{int(q * 100)}") for q in quantiles
    ]
    ...
    base = alt.Chart(stats_df).encode(x=alt.X("year:O", title="Year"))

    # Layer 1: Outer Ribbon (5th - 95th)
    ribbon_outer = base.mark_area(opacity=0.15, color="#2c7bb6").encode(
        y=alt.Y("q5:Q", title="Value"), y2="q95:Q"
    )
    ...
    median_line = base.mark_line(color="white", size=2.5).encode(y="q50:Q")

    # Layer 4: Unweighted Mean Line (Dashed)
    mean_line = base.mark_line(color="#d7191c", strokeDash=[5, 5], size=2).encode(
        y="mean_val:Q"
    )
    ...
    # Combine everything
    chart = (
        (ribbon_outer + ribbon_inner + median_line + mean_line + tooltips)
        .properties(width=700, height=450, title="AEWR: Mean vs Median and Quantiles")
        .interactive()
    )
...
def _(aewr_1990_2025, pl):
    ...
    ecdf_df = processed_df.sort(["year", "aewr_change"]).with_columns(
...
def _(alt, ecdf_df):
    ...
    ecdf_line = (
        alt.Chart()
        .mark_line(interpolate="step-after", color="#2c7bb6")
        .encode(
    ...
    faceted_chart = (
        alt.layer(ecdf_line, rule, data=ecdf_df)
        .facet(
        ...
        .properties(title="Yearly Empirical CDF of aewr Changes")
        .resolve_scale(

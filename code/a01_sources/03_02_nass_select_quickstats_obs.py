# Purpose: Select and harmonize the NASS observations used downstream.
# Inputs: QuickStats partitions and the observation-selection specification.
# Outputs: qs_census_selected_obs.parquet and qs_survey_selected_obs.parquet.
...
def _():
    import marimo as mo
    from h2a.paths import INTERMEDIATE, CACHE
    import os
    import polars as pl
    import dspy
...
def _(binary_path, pl):
    survey_crops = pl.scan_parquet(binary_path / "qs_survey_crops.parquet")
    census_crops = pl.scan_parquet(binary_path / "qs_census_crops.parquet")
    ...
    census_crops = (
        census_crops.with_columns(pl.col(pl.Categorical).cast(pl.String))
        .filter(
            pl.col("year") >= 2007,
            pl.col("agg_level_desc").is_in(["NATIONAL", "STATE", "COUNTY"]),
            pl.col("unit_desc") != "OPERATIONS",
    ...
    survey_crops = (
        survey_crops.with_columns(pl.col(pl.Categorical).cast(pl.String))
        .filter(
            pl.col("year") >= 2007,
            pl.col("agg_level_desc").is_in(["NATIONAL", "STATE", "COUNTY"]),
            pl.col("unit_desc") != "OPERATIONS",
    ...
    census_crops = census_crops.with_columns(
        pl.when(
            (pl.col("unit_desc") == "ACRES")
            & (pl.col("statisticcat_desc") != "AREA NON-BEARING")
        )
        .then(pl.lit("area"))
        .when(pl.col("statisticcat_desc") == "PRODUCTION")
        .then(pl.lit("production"))
        .when(pl.col("statisticcat_desc") == "YIELD")
        .then(pl.lit("yield"))
        .otherwise(None)
    ...
    survey_crops = survey_crops.with_columns(
        pl.when(pl.col("unit_desc").is_in(["ACRES", "ACRES WHERE TAPS SET"]))
        .then(pl.lit("area"))
        .when(pl.col("statisticcat_desc") == "PRICE RECEIVED")
        .then(pl.lit("price"))
        .when(pl.col("statisticcat_desc") == "PRODUCTION")
        .then(pl.lit("production"))
        .when(pl.col("statisticcat_desc") == "YIELD")
...
def _(BaseModel, Field, List):
    ...
    class CropDefinition(BaseModel):
        ...
        util_practice_desc: str = Field(
            description="The utilization practice, e.g., GRAIN"
        )
        reasoning: str = Field(
    ...
    test_cases = [
        {
            "group": "FIELD CROPS",
            "commodity": "WHEAT",
        },  # Multiple Classes (Winter/Spring)
        ...
        {"group": "VEGETABLES", "commodity": "BEANS"},  # Overlapping Class names
        {
            "group": "VEGETABLES",
            "commodity": "MELONS",
        },  # Class identifies distinct crops
    ]
...
def _(pl, summary_t):
    ...
    # Get summary stats from Polars table
    def markdown_summary_table(group_val, comm_val):
        summary_df = summary_t.filter(
            pl.col("group_desc") == group_val,
            pl.col("commodity_desc") == comm_val,
            # Only include categorized observations to save on tokens
            pl.col("observation_type").is_not_null(),
        ).collect()
...
def _(summary_t):
    ...
    all_tasks = (
        summary_t.select("group_desc", "commodity_desc").unique().collect().to_dicts()
    )
    return (all_tasks,)
...
def _(json, mapping_file, pl):
    ...
    nass_obs_selection = (
        ...
        .unnest(
            "output"
        )  # output only has 1 item inside: definitions of crops selected
        .explode(
            "definitions", empty_as_null=True
...
def _(census_crops, nass_obs_selection, survey_crops):
    ...
    selection = nass_obs_selection.lazy()

    census_selected_crops = census_crops.join(
        selection, how="inner", on=join_columns
    ).collect()

    survey_selected_crops = survey_crops.join(
        selection, how="inner", on=join_columns
    ).collect()

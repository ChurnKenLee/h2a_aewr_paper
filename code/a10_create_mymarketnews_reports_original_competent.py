...
@app.cell
def _(api_auth, base_url, pd, requests):
    # Make request
...
def _(pd):
    ...
    terminal_markets = terminal_markets[
        terminal_markets["offices"] != "['Washington, DC International - SC']"
    ]
...
@app.cell
def _(api_auth, base_url, pd, requests, terminal_markets, time):
    ...
    for slug_id in terminal_markets["slug_id"]:
        ...
        # Define request
        request_url = base_url + str(
            slug_id
        )  # + '?q=' + 'report_begin_date=12/19/2024' + '&allSections=true'
    ...
    report_header_df.to_parquet(
        "../binaries/mymarketnews_terminal_report_headers.parquet", index=False
    )
    return
...
def _(Path, api_auth, base_url, pd, requests, terminal_markets, time):
    ...
    for _year in range(2004, 2025):
        ...
        for _slug_id in terminal_markets["slug_id"]:
            _slug_year_path = Path(
                f"../Data/mymarketnews_reports/mymarketnews_reports_{_year}/mymarketnews_terminal_market_reports_year_{_year}_slug_{_slug_id}.csv"
            )
            ...
            _request_url = (
                ...
                + str(_slug_id)
                + "?q="
                + f"report_begin_date=01/01/{_year}:12/31/{_year}"
                + "&allSections=true"
            )
            ...
            try:
                ...
                _r = requests.get(
                    url=_request_url, auth=api_auth
                )  # stagger each request by 5 sec
            _r_json = _r.json()
            ...
            _result_df.to_csv(
                f"../Data/mymarketnews_reports/mymarketnews_reports_{_year}/mymarketnews_terminal_market_reports_year_{_year}_slug_{_slug_id}.csv",
                index=False,
            )  # Make request  # Put response into dataframe  # terminal_report_details_df = pd.concat([terminal_report_details_df, result_df])
    return
...
def _(Path, api_auth, base_url, pd, requests, shipping_points, time):
    ...
    for _year in range(2004, 2025):
        ...
        for _slug_id in shipping_points["slug_id"]:
            _slug_year_path = Path(
                f"../Data/mymarketnews_reports/mymarketnews_reports_{_year}/mymarketnews_shipping_point_reports_year_{_year}_slug_{_slug_id}.csv"
            )
            if _slug_year_path.is_file():
                print(
                    "Slug ID", str(_slug_id), "year", str(_year), "already downloaded."
            ...
            _result_df.to_csv(
                f"../Data/mymarketnews_reports/mymarketnews_reports_{_year}/mymarketnews_shipping_point_reports_year_{_year}_slug_{_slug_id}.csv",
                index=False,
            )  # Pull report details using Slug ID  # Define request  # Make request  # Put response into dataframe  # shipping_point_report_details_df = pd.concat([shipping_point_report_details_df, result_df])
    return
...
def _(Path, pd):
    for _year in range(2004, 2025):
        ...
        _binary_year_path = Path(
            f"../binaries/mymarketnews_shipping_point_reports_{_year}.parquet"
        )
        if _binary_year_path.is_file():  # Skip if binary already created
            ...
            for _csv_path in Path(
                f"../Data/mymarketnews_reports/mymarketnews_reports_{_year}"
            ).iterdir():
...
def _(Path, pd):
    for _year in range(2004, 2025):
        _year_df = pd.DataFrame()
        _binary_year_path = Path(
            f"../binaries/mymarketnews_terminal_market_reports_{_year}.parquet"
        )
        if _binary_year_path.is_file():  # Skip if binary already created
            print(str(_year), "already converted")
            continue

...
qs_census_crops <- qs_census_crops %>%
  mutate(
    numeric_value = str_replace_all(value, pattern = ",", replacement = "")
  ) %>%
...
qs_census_crops <- qs_census_crops %>%
  mutate(
    is_area = (unit_desc == "ACRES") & (statisticcat_desc != "AREA NON-BEARING")
  ) %>%
...
qs_survey_crops <- qs_survey_crops %>%
  mutate(
    is_area = (unit_desc == "ACRES") | (unit_desc == "ACRES WHERE TAPS SET")
  ) %>%
  ...
  mutate(
    keep_this = case_when(
      group_desc != "HORTICULTURE" &
        prodn_practice_desc == "ALL PRODUCTION PRACTICES" ~ TRUE,
      group_desc != "HORTICULTURE" &
        prodn_practice_desc == "IN THE OPEN" ~ TRUE,
...
census_class_max %>% write_csv('test.csv')
...
selected_crops_obs %>%
  write_parquet(here("binaries", "qs_selected_crops_obs.parquet"))
...
qs_census_economics <- read_parquet(here(
  "binaries",
  "qs_census_economics.parquet"
)) %>%
  clean_names()
...
qs_census_selected_obs %>%
  write_parquet(here("binaries", "qs_census_selected_obs.parquet"))
qs_survey_selected_obs %>%
  write_parquet(here("binaries", "qs_survey_selected_obs.parquet"))

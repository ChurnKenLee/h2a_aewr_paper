...
quickstats_census <- read_parquet(here(
  "binaries",
  "qs_census_selected_obs.parquet"
))
quickstats_survey <- read_parquet(here(
  "binaries",
  "qs_survey_selected_obs.parquet"
))
...
combined <- combined %>%
  ...
  mutate(
    imputed_production_unit = case_when(
...
combined <- combined %>%
  mutate(
    impute_production_here = (is.na(production) & !is.na(imputed_production))
  ) %>%
  mutate(
    production = if_else(impute_production_here, imputed_production, production)
  ) %>%
  mutate(
    production_unit = if_else(
      impute_production_here,
      imputed_production_unit,
      production_unit
    )
  )
...
combined <- combined %>%
  mutate(
    production = case_when(
      ...
      price_unit == "$ / CWT" & production_unit == "TONS" ~ production *
        2000 /
        112, # tons to cwt
  ...
  mutate(
    production = case_when(
      ...
      commodity_desc == "CRANBERRIES" &
        price_unit == "$ / BARREL" &
        production_unit == "BU" ~ production * 32 / 100,
      commodity_desc == "CRANBERRIES" &
        price_unit == "$ / BARREL" &
        production_unit == "LB" ~ production / 100,
      commodity_desc == "CRANBERRIES" &
        price_unit == "$ / BARREL" &
        production_unit == "CWT" ~ production * 112 / 32,
      commodity_desc == "CRANBERRIES" &
        price_unit == "$ / BARREL" &
        production_unit == "TONS" ~ production * 2000 / 100,
      # Potatoes: 1 bushel = 60 lbs
      commodity_desc == "POTATOES" &
        price_unit == "$ / CWT" &
        production_unit == "BU" ~ production * 60 / 112,
      # Sweet potatoes: 1 bushel = 55 lbs
      commodity_desc == "SWEET POTATOES" &
        price_unit == "$ / CWT" &
        production_unit == "BU" ~ production * 55 / 112,
      # Apples: 1 bushel = 40 lbs
      commodity_desc == "APPLES" &
        price_unit == "$ / LB" &
        production_unit == "BU" ~ production * 40,
      # Peaches: 1 bushel = 48 lbs
      commodity_desc == "PEACHES" &
        price_unit == "$ / TON" &
        production_unit == "BU" ~ production * 48 / 2000,
      # Chile peppers: 1 bushel = 25-30 lbs
      commodity_desc == "PEPPERS" &
        class_desc == "CHILE" &
        price_unit == "$ / CWT" &
        production_unit == "BU" ~ production * 27.5 / 112,
      # Green peas: 1 bushel = 28 lbs
      commodity_desc == "PEAS" &
        class_desc == "GREEN" &
        price_unit == "$ / TON" &
        production_unit == "BU" ~ production * 28 / 2000,
      .default = production
    )
  ) %>%
  mutate(
    production_unit = case_when(
      ...
      commodity_desc == "CRANBERRIES" &
        price_unit == "$ / BARREL" &
        production_unit == "BU" ~ "BARRELS",
      commodity_desc == "CRANBERRIES" &
        price_unit == "$ / BARREL" &
        production_unit == "LB" ~ "BARRELS",
      commodity_desc == "CRANBERRIES" &
        price_unit == "$ / BARREL" &
        production_unit == "CWT" ~ "BARRELS",
      commodity_desc == "CRANBERRIES" &
        price_unit == "$ / BARREL" &
        production_unit == "TONS" ~ "BARRELS",
      # Potatoes: 1 bushel = 60 lbs
      commodity_desc == "POTATOES" &
        price_unit == "$ / CWT" &
        production_unit == "BU" ~ "CWT",
      ...
      commodity_desc == "APPLES" &
        price_unit == "$ / LB" &
        production_unit == "BU" ~ "LB",
      # Peaches: 1 bushel = 48 lbs
      commodity_desc == "PEACHES" &
        price_unit == "$ / TON" &
        production_unit == "BU" ~ "TONS",
      # Chile peppers: 1 bushel = 25-30 lbs
      commodity_desc == "PEPPERS" &
        class_desc == "CHILE" &
        price_unit == "$ / CWT" &
...
# Export
combined %>%
  write_parquet(here("Data Int", "county_price_output.parquet")) %>%

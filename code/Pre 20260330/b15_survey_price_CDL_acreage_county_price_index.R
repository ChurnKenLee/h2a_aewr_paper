...
census_crops_list_df <- census_crops_list_df %>%
  mutate(
    common_name = case_when(
      ...
      short_desc ==
        "BEANS, DRY EDIBLE, (EXCL CHICKPEAS & LIMA) - ACRES HARVESTED" ~ "dry beans",
      ...
      short_desc ==
        "BLACKBERRIES, INCL DEWBERRIES & MARIONBERRIES - ACRES GROWN" ~ "caneberries",
      ...
      short_desc ==
        "CHERRIES, SWEET - ACRES BEARING & NON-BEARING" ~ "cherries",
      ...
      short_desc ==
        "CUT CHRISTMAS TREES - ACRES IN PRODUCTION" ~ "christmas trees",
      ...
      short_desc ==
        "ESCAROLE & ENDIVE - ACRES HARVESTED" ~ "escarole and endive",
      short_desc == "FIGS - ACRES BEARING & NON-BEARING" ~ "figs",
      short_desc == "FLAXSEED - ACRES HARVESTED" ~ "flaxseed",
      short_desc ==
        "FLORICULTURE TOTALS, IN THE OPEN - ACRES IN PRODUCTION" ~ "floriculture",
      short_desc ==
        "FLORICULTURE TOTALS, UNDER PROTECTION - SQ FT IN PRODUCTION" ~ "floriculture",
      short_desc ==
        "FLORICULTURE, OTHER, IN THE OPEN - ACRES IN PRODUCTION" ~ "floriculture",
      short_desc ==
        "FLORICULTURE, OTHER, UNDER PROTECTION - SQ FT IN PRODUCTION" ~ "floriculture",
      ...
      short_desc ==
        "NURSERY TOTALS, IN THE OPEN - ACRES IN PRODUCTION" ~ "nursery",
      short_desc ==
        "NURSERY TOTALS, UNDER PROTECTION - SQ FT IN PRODUCTION" ~ "nursery",
      ...
      short_desc ==
        "PASSION FRUIT - ACRES BEARING & NON-BEARING" ~ "passion fruit",
      ...
      short_desc ==
        "PEAS, DRY, SOUTHERN (COWPEAS) - ACRES HARVESTED" ~ "dry peas",
      short_desc ==
        "PEAS, CHINESE (SUGAR & SNOW) - ACRES HARVESTED" ~ "green peas",
      short_desc ==
        "PEAS, GREEN, (EXCL SOUTHERN) - ACRES HARVESTED" ~ "green peas",
      short_desc ==
        "PEAS, GREEN, SOUTHERN (COWPEAS) - ACRES HARVESTED" ~ "green peas",
      ...
      short_desc ==
        "PLUM-APRICOT HYBRIDS, INCL PLUMCOTS & PLUOTS - ACRES BEARING & NON-BEARING" ~ "plum-apricot hybrids",
      short_desc == "PLUMS - ACRES BEARING & NON-BEARING" ~ "plums and prunes",
      short_desc == "PRUNES - ACRES BEARING & NON-BEARING" ~ "plums and prunes",
      short_desc ==
        "POMEGRANATES - ACRES BEARING & NON-BEARING" ~ "pomegranates",
      ...
      short_desc ==
        "TOMATOES, UNDER PROTECTION - SQ FT IN PRODUCTION" ~ "tomatoes",
      short_desc == "TRITICALE - ACRES HARVESTED" ~ "triticale",
      short_desc == "TURNIPS - ACRES HARVESTED" ~ "turnips",
      short_desc ==
        "WALNUTS, ENGLISH - ACRES BEARING & NON-BEARING" ~ "walnuts",
...
census_crops_list_df <- census_crops_list_df %>%
  mutate(
    common_name = case_when(
      ...
      short_desc ==
        "PEAS, GREEN, SOUTHERN (COWPEAS) - ACRES HARVESTED" ~ "peas",
      .default = common_name
    )
  )
...
categorization_table %>%
  write_csv(here("Data Int", "croplandcros_cdl_crop_category.csv"))
  ...
  mutate(
    common_name = case_when(
      commodity_element == "Almonds" &
        market_segment == "All" &
        geographic_extent == "California" ~ "almonds", # All US almonds are grown in California
      commodity_element == "Apples" &
        market_segment == "All" &
        geographic_extent == "United States" ~ "apples",
      commodity_element == "Apricots" &
        market_segment == "All" &
        geographic_extent == "United States" ~ "apricots",
      commodity_element == "Artichokes" &
        market_segment == "Dual, fresh and processing" &
        geographic_extent == "United States" ~ "artichokes", # The All Uses market segment for artichoke rounds everything to 1M CWT for the US
      commodity_element == "Asparagus" &
        (market_segment == "Fresh" | market_segment == "Processing") &
        geographic_extent == "United States" ~ "asparagus",
      commodity_element == "Avocados" &
        market_segment == "All" &
        geographic_extent == "United States" ~ "avocados",
      commodity_element == "Bananas" &
        market_segment == "All" &
        geographic_extent == "United States" ~ "bananas",
      commodity_element == "Broccoli" &
        (market_segment == "Fresh" | market_segment == "Processing") &
        geographic_extent == "United States" ~ "broccoli",
      commodity_element == "Brussels sprouts" &
        market_segment == "Dual, fresh and processing" &
        geographic_extent == "United States" ~ "brussels sprouts",
      commodity_element == "Cabbage" &
        (market_segment == "Fresh" | market_segment == "Processing") &
        geographic_extent == "United States" ~ "cabbage",
      commodity_element == "Cantaloupe" &
        market_segment == "All" &
        geographic_extent == "United States" ~ "cantaloupes",
      commodity_element == "Carrots" &
        (market_segment == "Fresh" | market_segment == "Processing") &
        geographic_extent == "United States" ~ "carrots",
      commodity_element == "Cauliflower" &
        (market_segment == "Fresh" | market_segment == "Processing") &
        geographic_extent == "United States" ~ "cauliflower",
      commodity_element == "Celery" &
        market_segment == "Dual, fresh and processing" &
        geographic_extent == "United States" ~ "celery",
      commodity_element == "Cranberries" &
        market_segment == "All" &
        geographic_extent == "United States" ~ "cranberries",
      commodity_element == "Cucumbers" &
        (market_segment == "Fresh" | market_segment == "Processing") &
        geographic_extent == "United States" ~ "cucumbers",
      commodity_element == "Dates" &
        market_segment == "All" &
        geographic_extent == "United States" ~ "dates",
      commodity_element == "Dry beans" &
        market_segment == "All uses" &
        geographic_extent == "United States" ~ "dry beans",
      commodity_element == "Eggplant" &
        market_segment == "Dual, fresh and processing" &
        geographic_extent == "United States" ~ "eggplant",
      commodity_element == "Figs" &
        market_segment == "All" &
        geographic_extent == "California" ~ "figs", # All US figs are grown in California
      commodity_element == "Garlic" &
        market_segment == "Dual, fresh and processing" &
        geographic_extent == "United States" ~ "garlic",
      commodity_element == "Grapefruit" &
        market_segment == "All" &
        geographic_extent == "United States" ~ "grapefruit",
      commodity_element == "Grapes" &
        market_segment == "All" &
        geographic_extent == "United States" ~ "grapes",
      commodity_element == "Hazelnuts" &
        market_segment == "All" &
        geographic_extent == "United States" ~ "hazelnuts",
      commodity_element == "Honeydew" &
        market_segment == "All" &
        geographic_extent == "United States" ~ "honeydew",
      commodity_element == "Kiwifruit" &
        market_segment == "All" &
        geographic_extent == "California" ~ "kiwifruit", # All US kiwifruit are grown in California

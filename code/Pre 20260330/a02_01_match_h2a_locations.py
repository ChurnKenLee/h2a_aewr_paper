...
def _(addfips, binary_path, pl, process_dataframe):
    ...
    h2a_placeid = h2a_placeid.with_columns(
        common_county_fips = pl.struct(
            ['common_county', 'state_name']
        )
        .map_elements(add_fips_to_places_api_county, return_dtype=pl.String)
    )
...
def _(pl):
    def finalize_and_collapse(placeid_df, added_fips_df, original_df, group_keys):
        ...
        placeid_df = (placeid_df
            .with_columns(
                [pl.col(k).cast(pl.String).fill_null('') for k in ['xcity', 'xstate', 'county_list', 'xzip']]
            ))
        added_fips_df = (
            added_fips_df
                .with_columns(
        ...
        added_fips_and_placeid = added_fips_df.join(
            placeid_df,
            on=['xcity', 'xstate', 'county_list', 'xzip'],
            how='left'
        ).fill_null('')

        # Select locally matched FIPS > Places API FIPS
        added_fips_and_placeid = (
            added_fips_and_placeid.with_columns(
                fips = pl
                    .when(pl.col('final_fips') != '')

...
for (v in feature_names) {
  ...
  x[is.na(x)] <- med
  sx <- sd(x)
  unit_features_scaled[[v]] <- if (!is.na(sx) && sx > 0) {
    (x - mean(x)) / sx
  } else {
    0
  }
...
for (block_name in names(feature_blocks)) {
  ...
  if (length(block_cols) > 0) {
    unit_features_scaled[block_cols] <- unit_features_scaled[block_cols] /
      sqrt(length(block_cols))
  }
}
...
county_year_units <- county_df %>%
  mutate(county_ansi = countyfips) %>%
  distinct(county_ansi, year, cz_aewr_region_fe, aewr_region_num)

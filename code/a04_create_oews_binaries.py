...
def _(Path, oews_path, re):
    ...
    for folder in Path(oews_path).iterdir():
        ...
        oews_path_dict[_year] = (
            _file_list  # All MSA files is prefixed with 'msa' or 'MSA', except for 1997, which has 'oes' prefix  # Non-metro areas are in separate file with 'BOS' prefix beginning in 2006
        )
...
def _(oews_path_dict, pd):
    ...
    for _year, _file_list in oews_path_dict.items():
        ...
        for file_path in _file_list:
            ...
            if (
                int(_year) == 2000
            ):  # Find first row that is non-empty in first column, which should be the header row
            ...
            file_list_df_dict[file_path.name] = (
                new_df  # For year 2000, occ_title has a typo
            )
        ...
        combined_df["a_mean"] = pd.to_numeric(
            combined_df["a_mean"], errors="coerce"
        )  # Convert to lowercase column titles for all years
        combined_df["area"] = combined_df["area"].str.strip(" ")
        combined_df["year"] = int(
            _year
        )  # For years 1998-2002, wage percentile columns had 'h_wpct' and 'a_wpct' prefixes
        oews_df = pd.concat(
            [oews_df, combined_df], ignore_index=True
        )  # Starting in 2019, area_name was changed to area_title  # Concat all dfs into one  # Convert total employment counts and mean wages into numeric  # Area code may have leading whitespace for 1999 and 2000  # Add year column  # Concat all years into one df
    return (oews_df,)

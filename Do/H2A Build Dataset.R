...
acs_immigrant_imputed <- read_parquet(paste0(folder_data, "acs_immigrant_cleaned.parquet"))
...
bea_caemp25n_data <- read_parquet(paste0(folder_data, "bea_caemp25n_data.parquet"))
bea_cainc45_data <- read_parquet(paste0(folder_data, "bea_cainc45_data.parquet"))
...
census_of_agriculture_cropland <- read_parquet(paste0(folder_data, "census_ag_cropland.parquet"))
census_pop_ests <- read_parquet(paste0(folder_data, "census_pop_ests.parquet"))
census_of_agriculture_cropland_base <- read_parquet(paste0(folder_data, "census_ag_cropland_2007.parquet"))
...
aewr_regions <- read.csv(file = paste0(folder_data, "aewr_regions.csv"), stringsAsFactors = F)
bea_caemp25n_data <- read_parquet(paste0(folder_data, "bea_caemp25n_data_year.parquet"))
bea_cainc45_data <- read_parquet(paste0(folder_data, "bea_cainc45_data_year.parquet"))
...
state_border_pairs <- read.csv(file = paste0(folder_data, "state_border_pairs.csv"), stringsAsFactors = F)
border_counties_allmatches <- read.csv(file = paste0(folder_data, "border_counties_allmatches.csv"), stringsAsFactors = F)
census_of_agriculture_cropland <- read_parquet(paste0(folder_data, "census_ag_cropland_year.parquet"))

census_pop_ests <- read_parquet(paste0(folder_data, "census_pop_ests_year.parquet"))

census_of_agriculture_cropland_base <- read_parquet(file = paste0(folder_data, "census_ag_cropland_2007_year.parquet"))
...
border_df %>% write_parquet(paste0(folder_data, "border_df_analysis_year.parquet"))
...
census_of_agriculture_cropland_base <- read_parquet(paste0(folder_data, "census_ag_cropland_2007_year.parquet"))
...
state_min <- state_min %>% 
  ...
  group_by(statefips) %>% 
  mutate(across(names(state_min)[3:11], 
                lag, 
                .names = "{.col}_l1"))

county_df <- merge(x = county_df, y = state_min, by = c("statefips", "year"), all.x = T, all.y = F)
...
county_df <- county_df %>% 
  mutate(aewr_state_ag_ppi_l1 = aewr_ppi_l1 - prevailing_ag_min_wage_ppi_l1,
         aewr_state_ag_ppi = aewr_ppi - prevailing_ag_min_wage_ppi)

county_df <- county_df %>% 
  mutate(ln_aewr_state_ag_ppi = log(aewr_state_ag_ppi),
...
county_df <- merge(x = county_df, y = county_df_farmempbase, by = "countyfips", all.x = T, all.y = F)

county_df <- county_df %>%
  mutate(h2a_req_share_farm_workers_start_year = nbr_workers_requested_start_year / emp_farm,
         h2a_cert_share_farm_workers_start_year = nbr_workers_certified_start_year / emp_farm,
         h2a_req_share_farm_workers_2011_start_year = nbr_workers_requested_start_year / emp_farm_2011,
         h2a_cert_share_farm_workers_2011_start_year = nbr_workers_certified_start_year / emp_farm_2011) # add h2a apps per farm later
...
county_df$h2a_req_share_farm_workers_2011_start_year[is.infinite(county_df$h2a_req_share_farm_workers_2011_start_year)] <- NA
county_df$h2a_cert_share_farm_workers_2011_start_year[is.infinite(county_df$h2a_cert_share_farm_workers_2011_start_year)] <- NA
...
share_cuts <- quantile(h2a_use_df$h2a_cert_share_farm_workers_start_year, probs = c(.5, .66, .75), na.rm = T)


h2a_use_df <- h2a_use_df %>% 
  mutate(high_h2a_count_50 = ifelse(nbr_workers_certified_start_year > count_cuts[1], 1, 0),
         high_h2a_count_66 = ifelse(nbr_workers_certified_start_year > count_cuts[2], 1, 0),
         high_h2a_count_75 = ifelse(nbr_workers_certified_start_year > count_cuts[3], 1, 0),
         high_h2a_share_50 = ifelse(h2a_cert_share_farm_workers_start_year > share_cuts[1] & !is.na(h2a_cert_share_farm_workers_start_year), 1, 0),
         high_h2a_share_66 = ifelse(h2a_cert_share_farm_workers_start_year > share_cuts[2] & !is.na(h2a_cert_share_farm_workers_start_year), 1, 0),
         high_h2a_share_75 = ifelse(h2a_cert_share_farm_workers_start_year > share_cuts[3] & !is.na(h2a_cert_share_farm_workers_start_year), 1, 0))
...
county_df <- county_df %>% 
  mutate(high_h2a_share_75_inverse = ifelse(high_h2a_share_75 == 0, 1, 0),
...
county_df %>% write_parquet(paste0(folder_data, "county_df_analysis_year.parquet"))

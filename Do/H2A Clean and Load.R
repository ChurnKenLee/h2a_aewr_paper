...
census_of_agriculture_cropland %>%
  write_parquet(paste0(folder_data, "census_ag_cropland.parquet"))
...
census_of_agriculture_cropland_base %>%
  write_parquet(paste0(folder_data, "census_ag_cropland_2007.parquet"))
...
acs_qcew_data <- read_parquet(
  paste0(folder_data, "acs_qcew.parquet"),
  stringsAsFactors = F
)
...
acs_qcew_data <- acs_qcew_data %>%
  mutate(
    ...
    acs_crop_emp_clean = ifelse(
      !is.na(acs_animal_emp) & !is.na(acs_crop_emp),
      acs_crop_emp,
      ifelse(!is.na(acs_animal_emp) & is.na(acs_crop_emp), 0, NA)
    ),
    acs_animal_emp_clean = ifelse(
      !is.na(acs_animal_emp) & !is.na(acs_crop_emp),
      acs_animal_emp,
      ifelse(is.na(acs_animal_emp) & !is.na(acs_crop_emp), 0, NA)
    ),
    qcew_crop_emp_clean = ifelse(
      !is.na(qcew_animal_emp) & !is.na(qcew_crop_emp),
      qcew_crop_emp,
      ifelse(!is.na(qcew_animal_emp) & is.na(qcew_crop_emp), 0, NA)
    ),
    qcew_animal_emp_clean = ifelse(
      !is.na(qcew_animal_emp) & !is.na(qcew_crop_emp),
      qcew_animal_emp,
      ifelse(is.na(qcew_animal_emp) & !is.na(qcew_crop_emp), 0, NA)
    )
...
acs_qcew_data <- acs_qcew_data %>%
  mutate(
    empratio_crop_animal_acs = acs_crop_emp_clean / acs_animal_emp_clean,
    empratio_crop_animal_qcew = qcew_crop_emp_clean / qcew_animal_emp_clean,
    ln_empratio_crop_animal_acs = log(
      acs_crop_emp_clean / acs_animal_emp_clean
    ),
    ln_empratio_crop_animal_qcew = log(
      qcew_crop_emp_clean / qcew_animal_emp_clean
    ),
...
aewr_data <- read_parquet(
  file = paste0(folder_data, "aewr.parquet"),
  stringsAsFactors = F
)
...
aewr_data <- aewr_data %>%
  ...
  summarise(aewr_ppi = mean(aewr_ppi, na.rm = T), aewr = mean(aewr, na.rm = T))
...
bea_cainc45_data <- bea_cainc45_data %>%
  ...
  group_by(census_period, countyfips) %>%
  summarise(
    farm_cashanimal = mean(farm_cashanimal, na.rm = T),
...
acs_immigrant_imputed <- read_parquet(
  paste0(folder_data, "acs_immigrant_imputed.parquet"),
  stringsAsFactors = F
)
...
acs_immigrant_imputed <- acs_immigrant_imputed %>%
  mutate(
    prime_cat = ifelse(prime_age == T, "p", "np"),
    sex_cat = ifelse(sex == "male", "m", "f"),
    im_cat = ifelse(
      immigrant_type == "documented_immigrant",
      "doc",
...
acs_immigrant_imputed <- acs_immigrant_imputed %>%
  mutate(
    share_undoc = pop_undoc / pop_total,
...
write_parquet(
  acs_immigrant_imputed,
  paste0(folder_data, "acs_immigrant_cleaned.parquet")
)
...
border_counties_allmatches <- merge(
  x = border_counties_allmatches,
  y = cz_file_small,
  by = "fipscounty",
  all.x = T,
  all.y = F
...
cz_file_small <- cz_file_small %>%
  rename(fipsneighbor = fipscounty, cz_out10_neighbor = cz_out10) # side 2

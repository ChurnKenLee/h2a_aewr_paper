...
acs_immigrant_imputed <- readRDS(paste0(
  folder_data,
  "acs_immigrant_imputed.rds"
))
...
census_of_agriculture_cropland <- readRDS(
  file = paste0(folder_data, "census_ag_cropland.rds")
)

census_pop_ests <- readRDS(paste0(folder_data, "census_pop_ests.rds"))

census_of_agriculture_cropland_base <- readRDS(
  file = paste0(folder_data, "census_ag_cropland_2007.rds")
)
...
border_df <- merge(
  x = border_df,
  y = aewr_data,
  by = c("census_period", "state_abbrev"),
  all.x = T,
  all.y = F
)
...
aewr_regions_m <- aewr_regions %>%
  rename(
    aewr_region_num_neighbor = aewr_region_num,
    neighbor_abbrev = state_abbrev
  )
...
border_df$nbr_workers_requested_all_years[is.na(
  border_df$nbr_workers_requested_all_years
)] <- 0
border_df$nbr_workers_certified_all_years[is.na(
  border_df$nbr_workers_certified_all_years
)] <- 0
border_df$man_hours_requested_all_years[is.na(
  border_df$man_hours_requested_all_years
)] <- 0
border_df$man_hours_certified_all_years[is.na(
  border_df$man_hours_certified_all_years
)] <- 0
border_df$nbr_applications_all_years[is.na(
  border_df$nbr_applications_all_years
)] <- 0

border_df$nbr_workers_requested_start_year[is.na(
  border_df$nbr_workers_requested_start_year
)] <- 0
border_df$nbr_workers_certified_start_year[is.na(
  border_df$nbr_workers_certified_start_year
)] <- 0
border_df$man_hours_requested_start_year[is.na(
  border_df$man_hours_requested_start_year
)] <- 0
border_df$man_hours_certified_start_year[is.na(
  border_df$man_hours_certified_start_year
)] <- 0
border_df$nbr_applications_start_year[is.na(
  border_df$nbr_applications_start_year
)] <- 0

border_df$nbr_workers_requested_fiscal_year[is.na(
  border_df$nbr_workers_requested_fiscal_year
)] <- 0
border_df$nbr_workers_certified_fiscal_year[is.na(
  border_df$nbr_workers_certified_fiscal_year
)] <- 0
border_df$man_hours_requested_fiscal_year[is.na(
  border_df$man_hours_requested_fiscal_year
)] <- 0
border_df$man_hours_certified_fiscal_year[is.na(
  border_df$man_hours_certified_fiscal_year
)] <- 0
border_df$nbr_applications_fiscal_year[is.na(
  border_df$nbr_applications_fiscal_year
)] <- 0
...
border_df <- border_df %>%
  mutate(
    emp_pop_ratio = emp_tot / pop_total,
...
border_df$h2a_req_share_farm_workers_start_year[is.infinite(
  border_df$h2a_req_share_farm_workers_start_year
)] <- NA
border_df$h2a_cert_share_farm_workers_start_year[is.infinite(
  border_df$h2a_cert_share_farm_workers_start_year
)] <- NA
...
border_df <- border_df %>%
  mutate(
    aewr_border_sample = ifelse(
      aewr_region_num == aewr_region_num_neighbor,
      0,
      1
    )
  )
...
border_df <- border_df %>%
  mutate(
    lower_aewr_reg_num = ifelse(
      aewr_region_num < aewr_region_num_neighbor,
      aewr_region_num,
      ifelse(
        aewr_region_num > aewr_region_num_neighbor,
        aewr_region_num_neighbor,
        NA
      )
    )
  )
...
border_df <- border_df %>%
  mutate(
    aewr_region_border_id = ifelse(
      aewr_region_num == lower_aewr_reg_num,
      paste0(aewr_region_num, "_", aewr_region_num_neighbor),
      ifelse(
        aewr_region_num > lower_aewr_reg_num,
        paste0(aewr_region_num_neighbor, "_", aewr_region_num),
        NA
      )
    )
  )
...
border_df <- border_df %>%
  mutate(
    lower_cnty_fips = ifelse(
      countyfips < fipsneighbor,
      countyfips,
...
border_df <- border_df %>%
  mutate(
    pair_id = ifelse(
      countyfips == lower_cnty_fips,
      paste0(countyfips, "_", fipsneighbor),
      ifelse(
        countyfips > lower_cnty_fips,
        paste0(fipsneighbor, "_", countyfips),
        NA
      )
    )
  )
...
border_df$county_unique_fe <- with(
  border_df,
  interaction(as.factor(countyfips), aewr_region_border_id)
) # treats counties in separate clusters as separate
...
border_df$pairtime_fe <- with(
  border_df,
  interaction(as.factor(pair_id), census_period)
)
...
border_df$aewrbordertime_fe <- with(
  border_df,
  interaction(as.factor(aewr_region_border_id), census_period)
)

levels(border_df$aewrbordertime_fe) <- c(
  levels(border_df$aewrbordertime_fe),
  "0.0"
)
...
border_df <- border_df %>%
  mutate(
    aewr_border_side = ifelse(
      aewr_region_num == lower_aewr_reg_num & aewr_border_sample == 1,
      0,
      ifelse(
        aewr_region_num != lower_aewr_reg_num & aewr_border_sample == 1,
        1,
...
census_of_agriculture_cropland <- readRDS(
  file = paste0(folder_data, "census_ag_cropland_year.rds")
)

census_pop_ests <- readRDS(paste0(folder_data, "census_pop_ests_year.rds"))

census_of_agriculture_cropland_base <- readRDS(
  file = paste0(folder_data, "census_ag_cropland_2007_year.rds")
)
...
border_df <- border_df %>%
  mutate(
    ...
    aewr_ppi_diff_pct_l1 = (aewr_ppi_l1 - aewr_ppi_neighbor_l1) /
      aewr_ppi_neighbor_l1,
    ...
    aewr_ppi_diff_pct_l2 = (aewr_ppi_l2 - aewr_ppi_neighbor_l2) /
      aewr_ppi_neighbor_l2
  )
...
border_df$aewrbordertime_fe <- with(
  border_df,
  interaction(as.factor(aewr_region_border_id), year)
)
...
share_cuts <- quantile(
  h2a_use_df$h2a_req_share_farm_workers_start_year,
  probs = c(.5, .66, .75),
  na.rm = T
)
...
h2a_use_df <- h2a_use_df %>%
  mutate(
    high_h2a_count_50 = ifelse(
      nbr_workers_requested_start_year > count_cuts[1],
      1,
      0
    ),
    high_h2a_count_66 = ifelse(
      nbr_workers_requested_start_year > count_cuts[2],
      1,
      0
    ),
    high_h2a_count_75 = ifelse(
      nbr_workers_requested_start_year > count_cuts[3],
      1,
      0
    ),
    high_h2a_share_50 = ifelse(
      h2a_req_share_farm_workers_start_year > share_cuts[1] &
        !is.na(h2a_req_share_farm_workers_start_year),
      1,
      0
    ),
    high_h2a_share_66 = ifelse(
      h2a_req_share_farm_workers_start_year > share_cuts[2] &
        !is.na(h2a_req_share_farm_workers_start_year),
      1,
      0
    ),
    high_h2a_share_75 = ifelse(
      h2a_req_share_farm_workers_start_year > share_cuts[3] &
        !is.na(h2a_req_share_farm_workers_start_year),
      1,
      0
    )

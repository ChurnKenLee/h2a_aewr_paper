# Purpose: Download and cache USDA MyMarketNews report data by slug and year.
# Inputs: MyMarketNews credentials and configured report slugs.
# Outputs: report manifests, headers, and partitioned raw report Parquet files.
...
def _(manifest, pl):
    ...
    terminal_market_slugs = (
        manifest.filter(
            pl.col("market_types").list.contains("Terminal"),
            ~pl.col("offices").list.contains("Washington, DC International - SC"),
        )
        ...
        for year in range(2004, 2026):
            ...
            for slug in slug_list:
                ...
                if slug_json[1]["stats"]["totalRows"] == 0:
                    print(
                        f"No results available for {report_type} slug {slug} in {year}"
                    )

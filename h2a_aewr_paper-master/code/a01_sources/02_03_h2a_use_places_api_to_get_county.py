# Purpose: Resolve remaining H-2A locations through the Google Places API.
# Inputs: unmatched-location suggestions, location caches, and Google API credentials.
# Outputs: updated place-ID and address-component caches for H-2A matching.
...
def _(pl, state_abbr_to_name):
    ...
    def concat_location(city_col, county_col, state_col):
        concat_df = (
            pl.concat_list([city_col, county_col, state_col])
            .list.eval(
                pl.element().filter((pl.element() != "") & (pl.element().is_not_null()))
            )
            .list.join(", ")
        )
    ...
    def process_df(df):
        clean_names_df = df.with_columns(
            [
                ...
                pl.col("state")
                .map_elements(state_abbr_to_name, return_dtype=pl.String)
                .alias("state_name"),
                pl.col("state_suggested")
                .map_elements(state_abbr_to_name, return_dtype=pl.String)
                .alias("state_name_suggested"),
            ]
        ).with_columns(
            [
                # Concatenate columns
                concat_location(
                    pl.col("city"), pl.col("county"), pl.col("state_name")
                ).alias("original_location_name"),
                concat_location(
                    pl.col("city_suggested"),
                    pl.col("county_suggested"),
                    pl.col("state_name_suggested"),
...
def _(CACHE, client, json, json_path, pl, places_v1, process_df, time):
    ...
    add_b_df = pl.read_csv(
        json_path / "unmatched_add_b_with_suggestions.csv", infer_schema=False
    ).fill_null("")
    ...
    def rate_limited_queries(queries, qpm, save_frequency=50):
        ...
        for query in queries:
            ...
            response = client.search_text(
                request=request, metadata=[("x-goog-fieldmask", "places.id")]
            )
    ...
    h2a_original_locations_candidate_list = rate_limited_queries(
        h2a_original_locations_list, 600
    )
    h2a_suggested_locations_list = h2a_df["suggested_location_name"].to_list()
    h2a_suggested_locations_candidate_list = rate_limited_queries(
        h2a_suggested_locations_list, 600
    )
    ...
    add_b_original_locations_candidate_list = rate_limited_queries(
        add_b_original_locations_list, 600
    )
    add_b_suggested_locations_list = add_b_df["suggested_location_name"].to_list()
    add_b_suggested_locations_candidate_list = rate_limited_queries(
        add_b_suggested_locations_list, 600
    )
    ...
    def add_common_id_cols(df: pl.DataFrame) -> pl.DataFrame:
        df_with_common_id = df.with_columns(
            common_placeid=pl.col("original_location_placeid").list.set_intersection(
                "suggested_location_placeid"
            )
        ).with_columns(has_common_placeid=pl.col("common_placeid").list.len() > 0)
...
def _(add_b_df, h2a_df, pl):
    ...
    placeid_list = pl.concat(series_to_combine).drop_nulls().unique().to_list()
    return (placeid_list,)
...
def _(api_key, limits, places_v1, sleep_and_retry):
    ...
    def get_address_components_from_placeid(placeid):
        ...
        response = client.get_place(request=request, metadata=field_mask)
...
def _(
    ...
    if json_file_path.exists():
        ...
        print(
            f"Loaded {len(placeid_address_components_dict)} pre-existing Place API responses from JSON."
        )
    else:
    ...
    for _placeid in placeid_list:
        ...
        if _placeid not in placeid_address_components_dict:
            ...
            if new_calls % 50 == 0:
                ...
                print(
                    f"Checkpoint: Saved {len(placeid_address_components_dict)} responses to JSON."
                )
